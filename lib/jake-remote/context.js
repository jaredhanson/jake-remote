...
function Context(host) {
  // TODO: Rename this class to Executor
  this.host = host;
}
...
Context.prompts = function(fn) {
  _prompts.push(fn);
}

Context.prompts(require('./prompts/sudo')());
...
Context.prototype.sudo = function(command, options, callback) {
  terminal.info('['+ this.host  +']' + ' sudo: ' + command);
  this._exec('sudo ' + command, options, callback);
}

Context.prototype._exec = function(command, options, callback) {
